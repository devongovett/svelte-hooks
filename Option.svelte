<script>
  import {useOption} from '@react-aria/listbox';
  import {createHooks, useRef} from './react';
  import {attrs} from './attrs';

  export let state;
  export let item;

  let optionProps, ref;
  let update = createHooks();

  $: isDisabled = state.disabledKeys.has(item.key);
  $: isSelected = state.selectionManager.isSelected(item.key);
  $: isFocused = state.selectionManager.focusedKey === item.key;

  $: update(() => {
    ref = useRef();
    ({optionProps} = useOption(
      {
        key: item.key,
        isDisabled,
        isSelected,
        shouldSelectOnPressUp: true,
        shouldFocusOnHover: true
      },
      state,
      ref
    ));
  });
</script>

<template>
  <li
    use:attrs={optionProps}
    class="cursor-default select-none relative py-2 pl-3 pr-9 focus:outline-none"
    class:text-white={isFocused}
    class:text-gray={!isFocused}
    class:bg-blue-600={isFocused}
    class:font-semibold={isSelected}
    class:font-normal={!isSelected}
    bind:this={ref.current}>
    <div class="flex items-center space-x-3">
      <slot />
    </div>
    {#if isSelected}
      <span
        aria-hidden="true"
        class="absolute inset-y-0 right-0 flex  items-center pr-4"
        class:text-white={isFocused}
        class:text-gray-600={!isFocused}>
        <svg class="h-5 w-5 fill-current" viewBox="0 0 20 20">
          <path
            fill-rule="evenodd"
            d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
            clip-rule="evenodd"
          />
        </svg>
      </span>
    {/if}
  </li>
</template>
